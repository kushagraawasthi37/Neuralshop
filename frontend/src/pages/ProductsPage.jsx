      ...
      >
        ...
        <div style={{ position: "relative", aspectRatio: "3/4", background: "#0d0c0b", overflow: "hidden" }}>
          ...
          <button onClick={handleWishlist}
            ...
            <span style={{ fontSize: 14, color: wishlisted ? "#c9a96e" : "rgba(201,169,110,0.4)", lineHeight: 1 }}>{wishlisted ? "♥" : "♡"}</span>
          </button>
        </div>
...
export default function ProductsPage() {
  ...
  const activeFilters = [
    ...
    ...(priceMin > 0 || priceMax < 50000 ? [{ label: `₹${priceMin.toLocaleString("en-IN")} – ₹${priceMax.toLocaleString("en-IN")}`, clear: () => { setPriceMin(0); setPriceMax(50000); } }] : []),
  ];
  ...
  return (
    <div style={{ minHeight: "100vh", paddingTop: 80 }}>
      ...
      {/* Page Header */}
      <div style={{ padding: "48px 52px 32px", borderBottom: "1px solid rgba(201,169,110,0.08)" }}>
        <div style={{ fontSize: 9, letterSpacing: "0.28em", textTransform: "uppercase", color: "#c9a96e", display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          <span style={{ width: 24, height: 1, background: "#c9a96e", display: "inline-block" }} />
          Collection
        </div>
        <h1 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "clamp(32px,4vw,52px)", fontWeight: 300, color: "#f0e6d0", lineHeight: 1.1, marginBottom: 8 }}>
          {category && category !== "All" ? category : <><em style={{ fontStyle: "italic", color: "#c9a96e" }}>All</em> Products</>}
        </h1>
      ...
      <div style={{ display: "flex", padding: "0 52px" }}>
        {/* Sidebar */}
        <aside style={{ width: 240, flexShrink: 0, paddingTop: 40, paddingRight: 40, borderRight: "1px solid rgba(201,169,110,0.08)" }}>
          {/* Category */}
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              ...
              {activeFilters.length > 1 && (
                <button onClick={() => { setCategory("All"); setPriceMin(0); setPriceMax(50000); setSortBy(""); }}
              ...
              {totalPages > 1 && (
                <div style={{ display: "flex", justifyContent: "center", gap: 4, marginBottom: 48 }}>
                  <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1}
                    style={{ padding: "10px 18px", background: "none", border: "1px solid rgba(201,169,110,0.2)", color: "rgba(240,230,208,0.45)", cursor: page === 1 ? "default" : "pointer", fontSize: 11, letterSpacing: "0.1em", opacity: page === 1 ? 0.4 : 1 }}>
                    ← Prev
                  </button>
                  ...
                  <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages}
                    style={{ padding: "10px 18px", background: "none", border: "1px solid rgba(201,169,110,0.2)", color: "rgba(240,230,208,0.45)", cursor: page === totalPages ? "default" : "pointer", fontSize: 11, letterSpacing: "0.1em", opacity: page === totalPages ? 0.4 : 1 }}>
                    Next →
                  </button>
                </div>
              )}
