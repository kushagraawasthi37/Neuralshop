...
import { analyticsApi, adminOrdersApi, adminInventoryApi, adminReturnsApi } from "../../../api/admin";
  ...
  return (
    <div className="ns-content">
      <div className="page-header">
        ...
        <div className="page-title">Intelligence <em>Dashboard</em></div>
        <div className="page-sub">Real-time commerce pulse</div>
      </div>

      <div className="kpi-grid">
        {[
          { label: "Total Revenue", value: fmt(dashboard?.totalRevenue), gold: true, points: "0,28 15,22 30,30 45,18 60,24 75,12 90,18 105,8 120,14" },
          { label: "Total Orders", value: fmtNum(dashboard?.totalOrders), gold: false, points: "0,32 15,28 30,35 45,20 60,25 75,22 90,15 105,18 120,12" },
          { label: "Total Customers", value: fmtNum(dashboard?.totalCustomers), gold: false, points: "0,36 15,30 30,38 45,28 60,22 75,30 90,20 105,14 120,18" },
          { label: "Avg Order Value", value: fmt(dashboard?.avgOrderValue), gold: false, points: "0,30 15,35 30,28 45,32 60,22 75,26 90,20 105,22 120,16" },
        ].map(({ label, value, gold, points }) => (
          <div key={label} className="card">
            <div className="card-label">{label}</div>
            <div className={`card-value${gold ? " gold" : ""}`}>{value}</div>
            <svg className="sparkline" viewBox="0 0 120 48" style={{ marginTop: 12 }}>
              <polyline className="sparkline-area" points={`0,48 ${points} ${points.split(" ").at(-1).split(",")[0]},48`} />
      ...
      <div className="two-col mt-2">
          ...
          {salesLoading || salesChartData === null ? (
            ...
            <ResponsiveContainer width="100%" height={180}>
              <AreaChart data={salesChartData}>
                ...
                <Area type="monotone" dataKey="revenue" stroke="#c9a96e" strokeWidth={1.5} fill="url(#revGrad)" dot={false} />
              </AreaChart>
            </ResponsiveContainer>
          ) : (
            <div style={{ height: 180, display: "flex", alignItems: "center", justifyContent: "center", color: "var(--text-muted)", fontSize: 13 }}>
        ...
        <div className="card" style={{ padding: 28 }}>
          <div className="card-label" style={{ marginBottom: 24 }}>Order Status Distribution</div>
      ...
      <div className="two-col mt-2">
        <div className="card" style={{ padding: 0 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "20px 24px 16px" }}>
            <div className="card-label" style={{ marginBottom: 0 }}>Recent Orders</div>
            <button className="ns-btn ns-btn-ghost" style={{ padding: "6px 14px", fontSize: 10 }} onClick={() => onNavigate("orders")}>
              View All →
            </button>
          </div>
          <div className="table-wrap">
            <table className="ns-table">
              <thead>
                <tr><th>Order ID</th><th>Items</th><th>Amount</th><th>Status</th></tr>
              </thead>
              <tbody>
                {orders.slice(0, 5).map((o) => (
                  <tr key={o.id}>
                    <td className="primary"><span className="ns-code">#{(o.id || "").slice(0, 8)}</span></td>
                ...
                {orders.length === 0 && (
                  <tr><td colSpan={4} style={{ textAlign: "center", padding: "32px 16px", color: "var(--text-muted)" }}>No orders</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
          <div className="card" style={{ flex: 1 }}>
            <div className="card-label" style={{ marginBottom: 16 }}>Low Stock Alerts</div>
            {inventory.length === 0 ? (
              <div style={{ fontSize: 13, color: "var(--text-muted)" }}>No low stock items</div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {inventory.slice(0, 3).map((inv, i) => {
                  const avail = inv.availableStock ?? inv.totalStock;
                  const pct = inv.totalStock > 0 ? Math.round((avail / inv.totalStock) * 100) : 0;
                  return (
                    <div key={i}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                        <div>
                          <div style={{ fontSize: 13, color: "var(--champagne)" }}>{inv.productId?.slice?.(0, 20) || "—"}</div>
                          <div style={{ fontSize: 11, color: "var(--text-muted)" }}>Size {inv.size || "—"}</div>
                        </div>
          ...
          <div className="card" style={{ padding: 24 }}>
            <div className="card-label" style={{ marginBottom: 14 }}>Quick Stats</div>
            <div className="timeline">
              <div className="timeline-item">
                <div className="timeline-dot active" />
                <div className="timeline-content">
                  <div className="timeline-title">Pending orders: {fmtNum(pendingOrders)}</div>
              ...
              <div className="timeline-item">
                <div className="timeline-dot active" />
                <div className="timeline-content">
                  <div className="timeline-title">Low stock items: {fmtNum(inventory.length)}</div>
              ...
              <div className="timeline-item">
                ...
                <div className="timeline-content">
                  <div className="timeline-title">Pending returns: {fmtNum(pendingReturns)}</div>
