...
function ProductInventoryLookup({ showToast }) {
  ...
  const {
    ...
    queryFn: () =>
      adminInventoryApi.getById(lookupId).then((r) => r.data.data || r.data),
    enabled: !!lookupId,
    retry: false,
  });
  ...
  return (
    <div className="card" style={{ marginBottom: 2 }}>
      ...
      {error && (
        ...
        >
          Product not found or no inventory record.
        </div>
      )}
      {detail && (
        <div
...
export default function InventoryPanel({ showToast }) {
  ...
  const { data: inventory = [], isFetching } = useQuery({
    ...
    queryFn: () =>
      adminInventoryApi.getLowStock(threshold).then((r) => {
        const d = r.data.data;
  ...
  const handleJsonImport = async (e) => {
    ...
    try {
      ...
      if (!Array.isArray(inventory)) {
        return showToast(
          "Invalid JSON: expected array or { inventory: [...] }",
        );
      }
      ...
      showToast(
        err.response?.data?.message || "JSON import failed — invalid format",
      );
    }
  };
    ...
    <div className="ns-content">
      <div className="page-header">
        ...
        <div className="page-sub">
          {inventory.length} items at or below {threshold} units
        </div>
      </div>
      ...
      <div className="two-col mt-2" style={{ marginBottom: 2 }}>
        <div className="card">
          ...
          >
            Showing items with stock ≤ {threshold}
            {sliderVal !== threshold ? ` (applying ${sliderVal}…)` : ""}
          </div>
        </div>
        <div
        ...
        >
          <div className="stat-card">
            ...
            <div className="stat-val gold">{fmtNum(inventory.length)}</div>
          </div>
          <div className="stat-card">
            <div className="stat-label">Out of Stock</div>
            <div
              className="stat-val"
              style={{ color: "rgba(190,110,110,0.85)" }}
            >
              {fmtNum(outOfStock)}
            </div>
          </div>
          <div className="stat-card">
            ...
            <div className="stat-val">{fmtNum(inventory.length)}</div>
          </div>
          <div className="stat-card">
            <div className="stat-label">Reserve Stock</div>
            <div className="stat-val">{fmtNum(totalReserved)}</div>
          </div>
        </div>
      </div>

      <div
        className="inventory-import-row"
        style={{
          display: "flex",
          gap: 12,
          margin: "24px 0",
      ...
      >
        ...
        <span style={{ fontSize: 11, color: "var(--text-muted)" }}>
          CSV format: productId,size,totalStock · JSON: [
                    ...
                    <tr key={i}>
                      <td className="primary">
                        <span className="ns-code">
                          {(inv.productId || inv.id || "").slice(0, 18)}
                        </span>
                      </td>
                      <td>{inv.size || "—"}</td>
