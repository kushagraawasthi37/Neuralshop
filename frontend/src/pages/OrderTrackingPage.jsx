...
const STATUS_STEPS = [
  ...
  },
  {
    key: "PROCESSING",
    label: "Processing",
    desc: "The seller is preparing your order for shipment.",
  },
  {
    key: "SHIPPED",
    label: "Shipped",
  ...
  return (
    <div
      style={{
        ...
        width: 18,
        height: 18,
        border: `1px solid ${state !== "inactive" ? "#c9a96e" : "rgba(201,169,110,0.18)"}`,
        background:
          state === "done"
            ? "#c9a96e"
            : state === "active"
...
export default function OrderTrackingPage() {
  ...
  if (!order)
    return (
      ...
      >
        <div style={{ textAlign: "center", padding: "0 var(--page-px)" }}>
          <div
            style={{
  ...
  const progressPct =
    overallStatus === "CANCELLED"
      ? 0
      : Math.round(((currentStep + 1) / STATUS_STEPS.length) * 100);
  const address = order.address || order.shippingAddress;
  const returnDays = order.createdAt
    ? Math.max(
        0,
        30 - Math.floor((Date.now() - new Date(order.createdAt)) / 86400000),
      )
    : 30;
    ...
    <div style={{ minHeight: "100vh", paddingTop: "clamp(64px,10vw,100px)" }}>
      ...
      >
        ...
        <div style={{ padding: "clamp(32px,5vw,60px) 0 clamp(24px,4vw,48px)" }}>
          ...
          >
            Track Your{" "}
            <em style={{ fontStyle: "italic", color: "#c9a96e" }}>Order</em>
          </h1>
        </div>
        ...
        <div
          style={{
            display: "grid",
            gridTemplateColumns: "clamp(1px,1px,1px) 1fr", // overridden by class below
            gap: "clamp(20px,4vw,48px)",
            paddingBottom: 80,
            alignItems: "start",
          }}
          className="order-tracking-grid"
        >
          {/* Left: timeline */}
          <div
            style={{
          ...
          >
            ...
            >
              ...
              >
                {overallStatus === "CANCELLED"
                  ? "Order Cancelled"
                  : `Your order is ${overallStatus.toLowerCase()}`}
              </div>
            </div>
                      ...
                      style={{
                        ...
                        color:
                          state === "inactive"
                            ...
                            : "#f0e6d0",
                        fontWeight: state === "active" ? 400 : 300,
                        marginBottom: 4,
                      }}
                    >
                      {s.label}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              {items.map((item, i) => (
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    >
                      Qty: {item.quantity} · {item.status || overallStatus}
                    </div>
                  </div>
                  <div
                    style={{
            ...
            {/* Coupon (only if PENDING & no coupon applied) */}
                ...
                {couponMsg.text && (
                  <div
                    style={{
                      ...
                      fontSize: 11,
                      color: couponMsg.ok
                        ? "rgba(138,173,135,0.85)"
                        : "rgba(190,110,110,0.85)",
                    }}
                  >
                    {couponMsg.text}
                  </div>
            ...
            {address && (
              ...
              >
                ...
                <div
                  style={{ fontSize: 13, color: "#f0e6d0", marginBottom: 6 }}
                >
